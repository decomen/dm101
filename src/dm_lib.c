...
#include <math.h>
#include "dm_lib.h"
#include "../lib/dm_crc8.h"
#include "../lib/dm_crc32.h"
#include "../lib/dm_md5.h"
#include "../lib/dm_aes.h"
#include "../lib/cJSON.h"
...
static int32_t change_order( uint8_t *data, uint8_t len)
{
...
static uint32_t fomat_server_info(char *buf, struct Server_info server_info)
{
	if(buf == NULL)
		return -1;
	sprintf(buf,"{\
...
static char *show_head(const struct Head *pkg, const char *buf , char *showbuf)
{
	struct Head head;
	if(pkg != NULL){
		head = *pkg;
		...
		//dm101_debug("show_head pkg.payload_length = %ld\n", head.payload_length);
	}else{
...
static int32_t make_data_buf(uint8_t *outbuf , uint32_t outbuf_limit, const struct Data_pkg *pkg ,uint32_t *totalsize)
{
	...
	for(i = 0; i < pkg->data_cnt; i++){
	    sprintf(tmp_buf, "{\"sid\":%d,\"id\":\"%s\", \"value\":%lf, \"status\":%d},",\
			pkg->data[i].sid, pkg->data[i].id, pkg->data[i].value, pkg->data[i].status);
		tmp_len = strlen(outbuf);
...
//send read package 
static int32_t showoatbuf(char *tag, char *buf, int32_t len)
{
	int i;
...
static int32_t read_pkg(struct Dm101_context *context, struct Dm101pkg *dm101pkg)
{
	...
	if(context->getdata( context->args, (void*)head_buf, HEAD_LEN , F_READ,1000) < 0){
	...
	//dm101_debug("read head success : %d\r\n", nQueueLen(s_dm101_queue[0]));
   ...
   // show_head(&dm101pkg->head, NULL, NULL);
	...
	if(dm101pkg->head.payload_length > 0){
		...
		if(context->getdata( context->args, (void*)dm101pkg->pload_buf, dm101pkg->head.payload_length,F_READ,1000) < 0){
		...
		if(context->getdata( context->args, (void*)&dm101pkg->dm_crc32, sizeof(dm101pkg->dm_crc32),F_READ,1000) < 0){
...
static int32_t destory_dm101_context(struct Dm101_context *context)
{
...
static struct Dm101_context *create_dm101_context(void)
{
	...
	if (context != NULL) {
        ...
        context->recvbuf = (uint8_t *)dm101_malloc(DM101_SENDBUF_LEN);
        context->sendbuf = (uint8_t *)dm101_malloc(DM101_RECVBUF_LEN);
	...
	int (*getdata)(void *args,void *buf, uint32_t count, int8_t rw_flag, uint32_t timeout))
{
	...
	if(context == NULL || postdata == NULL|| getdata == NULL){
		return NULL;
	}
    context->post_get_reset = post_get_reset;
	...
	int32_t code, void *text, uint32_t text_len)
{
	...
	dm101pkg->pload_buf = context->sendbuf;
	dm101pkg->head.sn = sn++;
...
int32_t  send_pkg(struct Dm101_context *context, struct Dm101pkg *dm101pkg)
{
	...
	if(context->postdata( context->args, (void*)sendbuf, HEAD_LEN, F_WRITE) < 0){
	...
	if(head.payload_length > 0 && dm101pkg->pload_buf != NULL){
		...
		if(context->postdata( context->args, (void*)dm101pkg->pload_buf, head.payload_length, F_WRITE) < 0){
		...
		change_order_dm_crc32(dm101pkg);
		if(context->postdata( context->args, (void*)&dm101pkg->dm_crc32, sizeof(dm101pkg->dm_crc32),F_WRITE) < 0){
			dm101_debug("read dm_crc32 error\n");		
 			return -1;
		}
...
int client_handle(struct Dm101_context *context)
{
  ...
  //  dm101_debug("recv end\n");
    ...
    case CODE_SERVER_FIND: {
        ...
        dm101_debug("服务发现消息回复: type : 0x%04x, code = 0x%04x\r\n" , dm101pkg.head.type, dm101pkg.head.code);
		cJSON *json = cJSON_Parse(dm101pkg.pload_buf);
         uint32_t port = cJSON_GetDouble(json, "port", 0);
		 char *addr=cJSON_GetString(json, "addr", 0);
		 dm101_debug("ip地址：%s, 端口：%d\n",addr,port);
    ...
    case CODE_REPORT_INFO: {
        ...
        dm101_debug("上报设备信息回复: type : 0x%x, code = 0x%x\r\n" , dm101pkg.head.type, dm101pkg.head.code);
    ...
    case CODE_REPORT_DATA:{
        ...
        dm101_debug("上报实时数据回复: type : 0x%x, code = 0x%x\r\n" , dm101pkg.head.type, dm101pkg.head.code);
        break;
    }
     ...
     case CODE_HAERT:{
        ...
        dm101_debug("发送心跳包回复: type : 0x%x, code = 0x%x\r\n" , dm101pkg.head.type, dm101pkg.head.code);
        break;
    }
